...
export default function Features() {
    return (
        <section
            className="py-24 px-6 md:px-12 bg-white border-t border-slate-200/60 relative overflow-hidden"
            id="pipeline"
        >
            {/* Light elegant gradient */}
            <div className="absolute top-0 left-0 w-full h-64 bg-gradient-to-b from-slate-50 to-transparent opacity-60" />

            <div className="relative max-w-7xl mx-auto">
                <div className="text-center mb-20">
                    <h2 className="font-display text-5xl md:text-6xl font-bold mb-6 tracking-tight text-slate-900">
                        Core{' '}
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-indigo-600">
                            Pipeline
                        </span>
                    </h2>
                    <div className="w-24 h-1.5 bg-gradient-to-r from-teal-600 to-indigo-600 mx-auto rounded-full" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                    {features.map((feature, panelIndex) => (
                        <Card
                            key={panelIndex}
                            className="relative overflow-hidden rounded-3xl border-2 border-slate-200/60 bg-white/80 backdrop-blur-xl p-10 flex flex-col gap-8 min-h-[580px] transition-all duration-500 hover:-translate-y-3 hover:shadow-2xl hover:shadow-slate-300/50 hover:border-slate-300 group cursor-pointer"
                        >
                            {/* Light gradient overlay */}
                            <div className="absolute inset-0 bg-gradient-to-br from-white/60 to-transparent opacity-100 transition-opacity" />

                            <div className="relative z-10 flex flex-col h-full">
                                <h3 className={`font-display text-3xl md:text-4xl font-bold ${feature.titleColor} tracking-tight mb-3`}>
                                    {feature.title}
                                </h3>

                                <div className="flex-1 my-8 w-full aspect-square max-w-[320px] mx-auto">
                                ...
                                <div className="mt-auto">
                                    <PersonWithBubble text={feature.speech} position="below" align="center" />
                                </div>
                            </div>
                        </Card>
                    ))}
