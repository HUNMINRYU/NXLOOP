...
> 중요: `cloudbuild.pr.yaml`은 가능하면 `main`에도 커밋해 둔다.
> 새 브랜치를 `main`에서 파는 경우, 해당 파일이 브랜치에 없으면 PR 체크가 `not found`로 실패할 수 있다.
...
### 5.1 실제로 한 번 터졌던 케이스: `cloudbuild.pr.yaml not found`

- 증상: GitHub PR Checks에서 `File cloudbuild.pr.yaml not found`로 즉시 실패
- 원인: `cloudbuild.pr.yaml`이 로컬에만 있고 PR 브랜치 커밋에 포함되지 않음(untracked/미커밋)
- 해결:
