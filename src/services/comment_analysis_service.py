...
class CommentAnalysisService:
    ...
    def __init__(self, gemini_client=None) -> None:
        ...
        if self._gemini:
            ...
            self.pipeline = PipelineOrchestrator(
                source=TwoTowerSource(gemini_client),
                hydrator=FeatureHydrator(gemini_client),
                quality_filter=QualityFilter(),
                scorer=EngagementScorer(gemini_client),
    ...
    def analyze_with_ai(self, comments: list[dict]) -> dict:
        ...
        if self.pipeline:
            try:
                ...
                if loop and loop.is_running():
                    ...
                    pipeline_result = self._run_async(
                        self.pipeline.run_pipeline(comments)
                    )
                else:
                    pipeline_result = self._run_async(
                        self.pipeline.run_pipeline(comments)
        ...
        log_llm_request(
            "댓글 심층 분석",
            f"댓글 {len(sample_texts)}개 요약, 프롬프트 {len(combined_text)}자",
        )
        ...
        try:
            ...
            log_llm_response(
                "댓글 심층 분석", f"응답 {len(response_text)}자, 인사이트 추출 완료"
            )
